...
class TAPSchemaImporter:
    ...
    def import_schema(self, schema_name: str):
        ...
        if not schemas:
            ...
            logger.info("✓ Imported schema metadata")
    ...
    def import_tables(self, schema_name: str) -> list[str]:
        ...
        escaped_schema = self._escape_adql_string(schema_name)
        tables = self.query_tap_schema_table("tables", f"schema_name = '{escaped_schema}'")
    ...
    def import_columns(self, table_names: list[str], local_table_name: str = None):
        ...
        for table_name in table_names:
            ...
            columns = self.query_tap_schema_table("columns", f"table_name = '{escaped_table}'")
            ...
            for col_data in columns:
                ...
                # otherwise use original table name
                stored_table_name = (
                    local_table_name
                    if local_table_name and len(table_names) == 1
                    else col_data.get("table_name")
                )

                self.db.insert_column(
                    table_name=stored_table_name, column_name=col_data.get("column_name"), **kwargs
                )
                total_columns += 1
    ...
    def import_keys(self, table_names: list[str]):
        ...
        try:
            ...
            relevant_keys = [
                k for k in keys if k.get("from_table") in table_names or k.get("target_table") in table_names
            ]
            ...
            for key_data in relevant_keys:
                ...
                if key_id is not None:
                    ...
                    key_columns = self.query_tap_schema_table("key_columns", f"key_id = '{escaped_key_id}'")
                else:
                    key_columns = []
    ...
    def import_table_by_name(self, table_name: str, include_keys: bool = True, local_table_name: str = None):
        ...
        try:
            ...
            escaped_table = self._escape_adql_string(table_name)
            tables = self.query_tap_schema_table("tables", f"table_name = '{escaped_table}'")
...
Examples:
  ...
  python import_tap_schema.py --url https://irsa.ipac.caltech.edu/TAP \
                              --table gaia_dr3_source --local-table-name my_gaia_table
  ...
  python import_tap_schema.py --url <TAP_URL> --table <table> --no-keys
    ...
    target_group.add_argument(
        "--schema", help="Name of the schema/catalog to import (e.g., gaiadr3, wise_allwise)"
    )
    target_group.add_argument("--table", help="Name of a specific table to import (e.g., gaia_dr3_source)")
    ...
    parser.add_argument("--no-keys", action="store_true", help="Skip importing foreign key relationships")
    ...
    try:
        with TAPSchemaImporter(args.url, args.db_path) as importer:
            if args.schema:
                success = importer.import_schema_metadata(args.schema, include_keys=not args.no_keys)
                query_hint = f"SELECT * FROM tables WHERE schema_name = '{args.schema}';"
            else:
                success = importer.import_table_by_name(
                    args.table, include_keys=not args.no_keys, local_table_name=args.local_table_name
                )
